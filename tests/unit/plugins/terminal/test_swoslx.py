...
class TestTerminalModule:
    ...
    def test_on_become_already_in_privileged_mode(self, terminal_module):
        terminal_module._get_prompt.return_value = b"router(config)#"
    ...
    def test_on_become_not_in_privileged_mode_no_password(self, terminal_module):
        terminal_module._get_prompt.side_effect = [b"router>", b"router(config)#"]
        terminal_module.on_become()
    ...
    def test_on_become_fails_to_enter_privileged_mode(self, terminal_module):
        terminal_module._get_prompt.return_value = b"router>"
        terminal_module._exec_cli_command.side_effect = AnsibleConnectionFailure("unknown error")
        with pytest.raises(AnsibleConnectionFailure, match=re.escape("Failed to enable privileged user mode at prompt [router>] with error: unknown error")):
            terminal_module.on_become()

    def test_on_become_fails_to_change_prompt_after_enable(self, terminal_module):
        terminal_module._get_prompt.side_effect = [b"router>", b"router>"]
        with pytest.raises(AnsibleConnectionFailure, match=re.escape("Failed to enable privileged user mode, still at prompt [router>]")):
            terminal_module.on_become()
    ...
    def test_on_unbecome_in_privileged_in_config_mode(self, terminal_module):
        terminal_module._get_prompt.side_effect = [b"router(config)#", b"router>"]
        terminal_module.on_unbecome()
        assert terminal_module._exec_cli_command.call_args_list == [
            call(b"exit"),
            call(b"disable")
    ...
    def test_on_unbecome_disable_fails(self, terminal_module):
        ...
        with pytest.raises(AnsibleConnectionFailure, match=re.escape("Failed to disable privileged user mode with error: disable error")):
            terminal_module.on_unbecome()

    def test_on_unbecome_prompt_still_privileged(self, terminal_module):
        terminal_module._get_prompt.side_effect = [b"router#", b"router#"]
        with pytest.raises(AnsibleConnectionFailure, match=re.escape("Failed to disable privileged user mode, still at prompt [router#]")):
            terminal_module.on_unbecome()
